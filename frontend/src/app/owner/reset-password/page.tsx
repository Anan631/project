...
function PasswordStrengthIndicator({ password }: { password: string }) {
  const getStrength = (pwd: string) => {
    ...
    // Explicitly define the type for strengthMap to allow number indexing
...
function OwnerResetPasswordForm() {
  ...
  if (error) {
    return (
      <div className="text-center space-y-6 py-8 animate-fade-in">
        ...
        <Button asChild className="mt-4 bg-green-600 hover:bg-green-700">
          <Link href="/owner/forgot-password">طلب رابط جديد</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 text-right">
      <div>
        ...
        <div className="relative">
          <Input 
            ...
            {...register("password")} 
            className="bg-white focus:border-green-500 focus:ring-green-500 pr-10 py-2.5 rounded-lg border-2"
      ...
      <Button 
        type="submit" 
        className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 rounded-lg text-base mt-6" 
        disabled={isLoading}
      >
        {isLoading ? (
          <>
        ...
        <div className="w-full max-w-md">
          <Card className="bg-white/95 shadow-xl border-0 rounded-2xl">
            ...
            <CardFooter className="flex justify-center border-t border-gray-200">
              <Link href="/owner/forgot-password" className="text-green-600 hover:text-green-700 font-semibold text-sm transition-colors">
