...
export async function forgotPasswordAction(
  ...
  if (user && user.role === 'OWNER') {
    ...
    if (phone && user.phone !== phone) {
      ...
      // Email matches and user is owner (phone optional or correct)
      const tokenResult = await createPasswordResetTokenWithEmail(email, 'OWNER');

      if (tokenResult.success && tokenResult.token && tokenResult.userId) {
        await logAction('PASSWORD_RESET_EMAIL_REQUESTED', 'INFO', `Password reset email requested for owner: ${email}`, tokenResult.userId);
      }
    }
  } else {
    // User does not exist or is not an OWNER.
    // We log this for admin, but don't tell the user.
    await logAction(
      'PASSWORD_RESET_REQUEST_INVALID', 
      'INFO', 
      `Password reset requested for email: ${email}, but owner not found.`
    );
  }
