...
import { HardHat, Building2, CheckCircle2, Lock, ArrowRight } from "lucide-react";
...
export default function ConcreteCalculationsPage() {
  ...
  const [tabData, setTabData] = useState<Record<string, TabData>>({
    foundation: { length: '', width: '', height: '', completed: false },
            ...
            <TabsContent key={tab.id} value={tab.id}>
              <Card className="bg-white shadow-xl border-t-4 border-t-red-500">
                <CardHeader>
                  ...
                  <CardDescription>
                    {isLocked ? 'يجب إكمال الحساب السابق أولاً' : 'أدخل الأبعاد لحساب الحجم المطلوب'}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {isLocked ? (
