    ...
    <div className="container mx-auto py-12 px-4 min-h-screen bg-gray-50/50" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {calculationCards.map((card, index) => (
          <Link
            key={card.id}
            href={card.id === 'foundation' ? `/engineer/projects/${projectId}/concrete-cards/foundation` : card.id === 'cleaning-slab' ? `/engineer/projects/${projectId}/concrete-cards/cleaning-slab` : card.id === 'column-base' ? `/engineer/projects/${projectId}/concrete-cards/column-footings` : card.id === 'ground-bridges' ? `/engineer/projects/${projectId}/concrete-cards/ground-bridges` : card.id === 'ground-slab' ? `/engineer/projects/${projectId}/concrete-cards/ground-slab` : card.id === 'columns' ? `/engineer/projects/${projectId}/concrete-cards/columns` : '#'}
            className={cn(
              "group block h-full",
              card.id === 'foundation' || card.id === 'cleaning-slab' || card.id === 'column-base' || card.id === 'ground-bridges' || card.id === 'ground-slab' || card.id === 'columns' ? "cursor-pointer" : "cursor-not-allowed opacity-80"
            )}
          >
            <div className={cn(
